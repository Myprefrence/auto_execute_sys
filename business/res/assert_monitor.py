...
class QueryCredit:
    ...
    def query_guarantee_start_datetime(self, project_no):
        # time_value = ''
        guarantee_start_datetimes = '1900-01-01 00:00:00'
        guarantee_start_datetime = repay(self.mysql, self.xn, self.env).guarantee_start_datetime(project_no)
        if guarantee_start_datetime is None:
            time_value = guarantee_start_datetimes
        else:
            time_values = guarantee_start_datetime['guarantee_start_datetime']
            time_value = datetime.datetime.strftime(time_values,'%Y-%m-%d %H:%M:%S')
...
class AssertMonitor:
    ...
    def count_principal_balance(self, project_no: list, asset_org_no, quota_type=0):
        """
        根据项目号计算本金余额
        """
        sum_balance = 0
        ...
        if quota_type == 0:
            ...
            for projectNo in project_no:
                ...
                if cbs_loan_amt is not None:
                    ...
                    # 通道还款金额
                    pls_rpy_principal = repay(self.mysql, self.xn, self.env).pls_repay_record_principal(projectNo)['sum(repay_principal)']

                    # 通道代偿金额
                    pls_repay_principal = repay(self.mysql, self.xn, self.env).pls_compensate_principal(projectNo)['sum(compensate_principal)']
            ...
            for projectNo in project_no:
                ...
                if cbs_compare_quota is not None:
                    ...
                    pls_compare_quota =repay(self.mysql, self.xn, self.env).pls_compare_quota(projectNo, replace_warrant_start_time, time)['sum(loan_principal)']

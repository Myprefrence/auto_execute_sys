...
class fieldMap:
    ...
    def write_excel(self,data):
        ...
        # 标题
        title = ["序号", "cbs_value", "rms_value",  "res_value", "result"]
    ...
    def get_all_keys_and_values(self, target_data: dict, target_dict: dict):
        for param in target_data:
            try:
                json_data = json.loads(target_data[param])
            except (json.decoder.JSONDecodeError, TypeError):
                if type(target_data[param]) == dict:
                    self.get_all_keys_and_values(target_data[param], target_dict)
                elif type(target_data[param]) == list:
                    for list_data in target_data[param]:
                        if type(list_data) == dict:
                            for dict_data in list_data:
                                target_dict[dict_data] = list_data[dict_data]
                else:
                    target_dict[param] = target_data[param]
            else:
                if type(json_data) != dict:
                    target_dict[param] = target_data[param]
                else:
                    # print(json_data)
                    self.get_all_keys_and_values(json_data, target_dict)
        return target_dict
    ...
    def conn_handle(self,):
        ...
        list_f_new = []  # 最终结果
        if len(cbs_array) == 0 and len(rms_array) == 0 and len(res_array) == 0:
            print("值缺失")
        else:
            time.sleep(1.5)
        ...
        for length in range(len(cbs_array)):
            ...
            for list_i in (cbs_array, rms_array, res_array):
...
if __name__ =='__main__':
    ...
    cbs_file = os.path.dirname(os.path.abspath('')) + r'\config\rms.json'
    with open(cbs_file, 'r', encoding='utf-8') as f:
        json_param = f.read()
        json_data = json.loads(json_param)
        json_data = get_data.turn_json_to_dict(json_data)

        rms_d = get_data.get_valuelist_by_key(json_data, "outerCreditOrderId")
        print(rms_d)
